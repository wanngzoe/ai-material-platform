...
2. **hookNarration（钩子旁白）**：8-15字，口语化，疑问句或感叹句效果更好。如："下雨天，外卖小哥为啥不躲？"
3. **transition（过渡）**：用一句话说明"→素材"的逻辑关联，2-3句即可
4. **materialNarration（素材旁白）**：15-30字，清晰说明产品/服务价值

## 输出格式（严格JSON数组，不要任何其他文字）
[
  {{"hookScene": "具体画面描述，包含人物+动作+表情+场景细节", "hookNarration": "8-15字口语化旁白", "transition": "过渡说明", "materialNarration": "15-30字素材旁白"}}
]"""
                    else:
                        prompt = f"""生成恰好{count}条短剧广告引流素材创意。
...
2. **hookNarration（钩子旁白）**：8-15字，口语化，疑问句或感叹句效果更好。如："大爷的游戏操作，让我自愧不如"

## 输出格式（严格JSON数组，不要任何其他文字）
[
  {{"hookScene": "具体画面描述，包含人物+动作+表情+场景细节", "hookNarration": "8-15字口语化旁白"}}
]"""
...
## 色调与光影（开篇必须写）
先写整体风格，再写分镜：
- **色调**：低饱和暗绿/高对比冷色/暖黄复古等具体描述
- **光影**：自然顶光/侧光源/逆光剪影/烛火暖光等
- **氛围**：压抑/悬疑/温馨/紧张等
...
5. **字数参考**：新旁白总字数尽量接近{orig_char_count}字，误差±20%可接受
6. **口语化**：适合配音朗读，避免书面语
